...
let CloneModule (oldAsn1:AstRoot) (old:Asn1Module) (cons:Constructors<State>) (state:State)  = 
    ...
    let newTasses = thisModTasses |> List.map (fun tas -> {TypeAssignment.Name = tas.TasName.AsLoc ; c_name = ToC2 tas.TasName; ada_name = ToC2 tas.TasName; Type = tas.Type; Comments = [||]; baseType = None} )
